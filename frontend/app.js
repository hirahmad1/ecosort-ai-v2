...
function showPanel(name, opts = {}) {
  ...
  if (hero) hero.hidden = name !== "ecosort";
  ...
  document.body.classList.toggle("slim-chrome", name !== "ecosort");
...
function initNav() {
  ...
  const valid = ["matcher", "pantry", "meal-plan", "posts", "restaurants", "surplus", "ecosort", "messages", "profile", "admin"];
  if (hash && valid.includes(hash)) {
    showPanel(hash, { skipScroll: true });
  } else {
    showPanel("ecosort", { skipScroll: true });
  }
...
function openAuthDialog(mode = "login") {
  ...
  document.getElementById("auth-subtitle").textContent = isLogin
    ...
    : "Join EcoSort AI and start sorting waste responsibly.";
...
function pushFridgeNotifications(alerts) {
  ...
  const body =
    more > 0
      ...
      : first.message;
  try {
    const note = new Notification("EcoSort AI · Fridge alert", { body, tag: "petugram-fridge" });
    note.onclick = () => {
      window.focus();
...
/* ── EcoSort AI ── */
let ecosortFile = null;
const ECOSORT_CAT = {
  recycling: { icon: '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>', label: "Recycling", color: "#2980b9" },
  compost:   { icon: '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22c-4-4-8-7.5-8-12a8 8 0 0 1 16 0c0 4.5-4 8-8 12z"/><path d="M12 6v10"/><path d="M8 10c2 2 4 2 4 2"/><path d="M16 10c-2 2-4 2-4 2"/></svg>', label: "Compost",   color: "#27ae60" },
  landfill:  { icon: '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>', label: "Landfill",  color: "#7f8c8d" },
  "e-waste": { icon: '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/><polyline points="7 10 10 13 17 7"/></svg>', label: "E-Waste",   color: "#e67e22" },
  hazardous: { icon: '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>', label: "Hazardous",  color: "#c0392b" },
};

function ecosortStatus(msg, err = false) {
  setStatus(document.getElementById("ecosort-status"), msg, err);
}

function ecosortReset() {
  ecosortFile = null;
  const inp = document.getElementById("ecosort-input");
  if (inp) inp.value = "";
  const preview = document.getElementById("ecosort-preview");
  if (preview) { preview.classList.remove("is-loaded"); preview.removeAttribute("src"); }
  const broken = document.getElementById("ecosort-preview-broken");
  if (broken) broken.hidden = true;
  document.getElementById("ecosort-preview-wrap") && (document.getElementById("ecosort-preview-wrap").hidden = true);
  document.getElementById("ecosort-upload-zone") && (document.getElementById("ecosort-upload-zone").hidden = false);
  document.getElementById("ecosort-analyze-btn") && (document.getElementById("ecosort-analyze-btn").disabled = true);
  document.getElementById("ecosort-results") && (document.getElementById("ecosort-results").hidden = true);
  ecosortStatus("");
}

function ecosortRender(data) {
  const res = document.getElementById("ecosort-results");
  const items = document.getElementById("ecosort-items");
  const sum = document.getElementById("ecosort-summary");
  const tc = document.getElementById("ecosort-tips-card");
  const tl = document.getElementById("ecosort-tips-list");
  if (!res || !items) return;
  res.hidden = false;
  items.innerHTML = "";
  sum.textContent = data.message || `Identified ${data.count || 0} item(s)`;
  const arr = data.items || [];
  if (!arr.length) { items.innerHTML = `<p class="ecosort-empty">No waste items detected. Try a clearer photo.</p>`; if (tc) tc.hidden = true; return; }
  for (const it of arr) {
    const m = ECOSORT_CAT[it.category] || { icon: '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>', label: it.category, color: "#888" };
    const pct = Math.round((it.confidence || 0.8) * 100);
    const card = document.createElement("div");
    card.className = "ecosort-item-card";
    card.style.setProperty("--cat-color", m.color);
    card.innerHTML = `<div class="ecosort-item-head"><span class="ecosort-item-icon">${m.icon}</span><div class="ecosort-item-meta"><strong>${escapeHtml(it.name)}</strong><span class="ecosort-item-cat" style="background:${m.color}22;color:${m.color}">${m.label}</span></div><span class="ecosort-item-conf">${pct}%</span></div><div class="ecosort-item-body"><p><strong>Disposal:</strong> ${escapeHtml(it.disposal)}</p><p><strong>Tip:</strong> ${escapeHtml(it.tip)}</p></div>`;
    items.appendChild(card);
  }
  const tips = arr.filter(i => i.tip).map(i => i.tip);
  if (tips.length && tc && tl) { tc.hidden = false; tl.innerHTML = tips.map(t => `<li>${escapeHtml(t)}</li>`).join(""); } else if (tc) tc.hidden = true;
}

async function ecosortClassify() {
  if (!ecosortFile) return;
  const btn = document.getElementById("ecosort-analyze-btn");
  if (btn) btn.disabled = true;
  ecosortStatus("Analyzing waste… please wait.");
  const fd = new FormData(); fd.append("file", ecosortFile);
  try { const d = await api("/api/ecosort/classify", { method: "POST", body: fd }); ecosortStatus(""); ecosortRender(d); }
  catch (e) { ecosortStatus(e.message || "Classification failed.", true); }
  finally { if (btn) btn.disabled = false; }
}

function initEcoSort() {
  const inp = document.getElementById("ecosort-input");
  const btn = document.getElementById("ecosort-analyze-btn");
  const clr = document.getElementById("ecosort-clear-btn");
  if (inp) inp.addEventListener("change", () => {
    const f = inp.files?.[0]; if (!f) return; ecosortFile = f;
    ...
    r.onload = (e) => {
      ...
      document.getElementById("ecosort-preview-wrap").hidden = false;
      document.getElementById("ecosort-upload-zone").hidden = true;
      if (btn) btn.disabled = false;
      document.getElementById("ecosort-results")?.setAttribute("hidden", "");
      ecosortStatus("");
    };
    r.readAsDataURL(f);
  });
  // Handle broken/corrupted preview image
  const previewImg = document.getElementById("ecosort-preview");
  if (previewImg) {
  ...
  if (btn) btn.addEventListener("click", ecosortClassify);
  if (clr) clr.addEventListener("click", ecosortReset);
}
...
function renderPostMedia(post) {
  if (post.media_type === "video") {
    ...
    return `<video class="post-media" src="${escapeAttr(post.media_url)}" controls playsinline preload="metadata"></video>`;
  }
  return `<img class="post-media" src="${escapeAttr(post.media_url)}" alt="" loading="lazy" />`;
}
...
async function sharePost(postId) {
  ...
  try {
    if (navigator.share) {
      await navigator.share({ title: `EcoSort AI · ${post.username}`, text, url });
      return;
    }
  } catch {
    /* fall through */
  ...
  try {
    ...
    showToast("Share this post from EcoSort AI!", false);
  }
}
