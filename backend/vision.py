...
WASTE_CLASSIFY_PROMPT = (
    "You are an expert waste management AI called EcoSort. "
    "Look at this image carefully and identify the waste item(s) shown. "
    "For EACH distinct item, classify it into EXACTLY ONE category:\n"
    "- recycling (paper, cardboard, clean plastic bottles, glass, metal cans)\n"
    "- compost (food scraps, fruit peels, coffee grounds, eggshells, yard waste)\n"
    "- landfill (wrappers, chip bags, Styrofoam, broken ceramics, dirty plastic)\n"
    "- e-waste (batteries, phones, chargers, cables, light bulbs, electronics)\n"
    "- hazardous (paint, chemicals, motor oil, pesticides, medicine, solvents)\n\n"
    "Reply with ONLY a JSON object (no markdown):\n"
    '{"items": [{"name": "item name", "category": "category", "confidence": 0.95, "disposal": "how to dispose", "tip": "sustainability tip"}]}\n\n'
    "Rules: up to 5 items, confidence 0.5-0.99, one actionable disposal sentence, one short sustainability tip. "
    "If no waste detected, return {\"items\": []}"
)
...
class FoodScanner:
    ...
    def classify_waste(self, image_bytes: bytes, filename: str = "") -> dict[str, Any]:
        """Classify waste items in an image into recycling/compost/landfill/e-waste/hazardous."""
        prompt = WASTE_CLASSIFY_PROMPT
        if not image_bytes:
            raise ValueError("Empty image")
        if len(image_bytes) > 8 * 1024 * 1024:
            raise ValueError("Image too large (max 8 MB)")
        ...
        if not raw_text and self.token:
            ...
            for model in models:
                try:
                    if "blip" in model.lower() and "vl" not in model.lower():
                        continue
                    raw_text = self._chat_completions(model, image_bytes, filename, prompt)
                    if raw_text.strip():
                        source = f"huggingface:{model}"
                        break
                except Exception as exc:  # noqa: BLE001
                    warnings.append(f"{model}: {exc}")
        if not raw_text:
            raise RuntimeError("; ".join(warnings) or "Waste classification failed")
        items: list[dict[str, Any]] = []
        match = re.search(r"\{[\s\S]*\}", raw_text)
        if match:
            try:
                data = json.loads(match.group(0))
                valid_cats = {"recycling", "compost", "landfill", "e-waste", "hazardous"}
                for it in (data.get("items", []) or [])[:5]:
                    ...
                    if cat not in valid_cats:
                        continue
                    items.append({
                        "name": str(it.get("name", "Unknown item")).strip(),
                        "category": cat,
                        "confidence": round(float(it.get("confidence", 0.8)), 2),
                        "disposal": str(it.get("disposal", "Dispose responsibly.")).strip(),
                        "tip": str(it.get("tip", "Reduce, reuse, recycle.")).strip(),
                    })
            except (json.JSONDecodeError, ValueError):
                pass
        return {"items": items, "count": len(items), "raw": raw_text[:500], "source": source, "warnings": warnings}
