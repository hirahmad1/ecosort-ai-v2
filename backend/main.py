...
app = FastAPI(
    title="EcoSort AI — Smart Waste Sorting Platform",
    description=(
...
@app.post("/api/ecosort/classify")
async def ecosort_classify(file: UploadFile = File(...)):
    """EcoSort AI: classify waste items from a photo into disposal categories."""
    assert food_scanner is not None
    content_type = (file.content_type or "").lower()
    if content_type and not content_type.startswith("image/"):
        raise HTTPException(400, "Please upload an image (JPG, PNG, or WebP)")
    ...
    try:
        result = food_scanner.classify_waste(raw, filename=file.filename or "")
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(502, f"Waste classification failed: {exc}") from exc
    if not result.get("items"):
        return {**result, "message": "Could not identify waste items. Try a clearer photo."}
    return {**result, "message": f"Identified {result['count']} waste item(s). See disposal guidance below."}
